...
export function getCategoryTotals(data) {
  const filtered = data.filter((f) => f.myBudgetCategory !== "");
  ...
  const debitsLessRent = debitsSorted.filter((f) => f.category !== "Rent");

  //console.info(resultSorted);
  const totalExpenses = debitsSorted
    .map((num) => num.totalDebits)
    .reduce((a, b) => a + b, 0);
  const totalExpensesLessRent = debitsLessRent
    .map((num) => num.totalDebits)
    .reduce((a, b) => a + b, 0);
  ...
  const summary = {
    ...
    expensesLessRent: debitsLessRent,
    income: creditsSorted,
    totalExpenses: currencyFormat(totalExpenses, false),
    totalExpensesLessRent: currencyFormat(totalExpensesLessRent, false),
